...
def annotation(file, db, options, probe_mapping=None):
    variants, _ = parse_genotype_file_with_stats(str(file))
    if probe_mapping is not None:
        from allelio.probes import recover_probes
        variants = recover_probes(variants, file, probe_mapping, db)
...
def record_run(file, database, options=None, explain=False, model=None, top=20, probe_map=None):
    ...
    mapping_hash = checksum(probe_map) if probe_map else None
    mapping = load_mapping(probe_map) if probe_map else None
    software = code_identity()
    original = checksum(file)
    with references(database) as db:
        identity = reference_identity(db)
        evidence, results = annotation(file, db, options, mapping) if mapping is not None else annotation(file, db, options)
    ...
    if probe_map and checksum(probe_map) != mapping_hash:
        raise ValueError("Probe mapping changed during the run.")
    ...
    manifest = {'schema': SCHEMA, 'created_at': datetime.now(timezone.utc).isoformat(),
                ...
                'input': {'sha256': original}, 'probe_mapping_sha256': mapping_hash, 'references': identity, 'options': options,
...
def replay_run(manifest, file, database, verify_only=False, probe_map=None):
    """Verify immutable prerequisites before parsing; AI is never contacted."""
    validate_manifest(manifest)
    from allelio.probes import load_mapping
    if manifest.get("probe_mapping_sha256") != (checksum(probe_map) if probe_map else None):
        raise ValueError("Probe mapping is missing or changed.")
    mapping = load_mapping(probe_map) if probe_map else None
    ...
    with references(database) as db:
        ...
        evidence = None
        if not verify_only:
            evidence, _ = annotation(file, db, manifest['options'], mapping)
    ...
    if probe_map and checksum(probe_map) != manifest.get("probe_mapping_sha256"):
        raise ValueError("Probe mapping changed during replay.")
